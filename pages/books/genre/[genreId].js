  ...
  const favouriteGenreHandler = async () => {
    if (!user?.data) {
      snackbarCtx.addMessage({
        title: "Please login to save favourite genres",
        status: "invalid",
      });
      return;
    }
    ...
    if (library.genre === "saved")
      snackbarCtx.addMessage({
        title: "Genre saved in your library",
        status: "success",
      });
    else
      snackbarCtx.addMessage({
        title: "Genre removed from your library",
        status: "success",
      });
    setLoadingFavourite(false);
  };
...
export async function getStaticPaths() {
  ...
  const genreParams =
    ...
    genres.data.map((genre) => ({
      params: { genreId: genre.slug }, // Removed toString() method here
    }));
