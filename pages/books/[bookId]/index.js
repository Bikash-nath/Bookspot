...
function BookDetailPage(props) {
  ...
  useEffect(() => {
    if (!isFavourite && user?.data) {
      ;(async () => {
        ...
        if (!library.books)
          snackbarCtx.addMessage({ title: library, status: 'invalid' })
        else {
  ...
  const favouriteBookHandler = async () => {
    if (!user?.data) {
      snackbarCtx.addMessage({
        title: 'Please login to save favourite books',
        status: 'invalid',
      })
      return
    }
    ...
    if (library.book === 'saved')
      snackbarCtx.addMessage({
        title: 'Book saved in your library',
        status: 'success',
      })
    else
      snackbarCtx.addMessage({
  ...
  const editReviewHandler = (review) => {
    if (!user?.data) {
      snackbarCtx.addMessage({
        title: 'Please login to give book review',
        status: 'invalid',
      })
    } else {
  ...
  const audioBookHandler = () => {
    if (!book.format?.audiobook?.link) {
      snackbarCtx.addMessage({
        title: 'Sorry, Audiobook not avialabe!',
        status: 'fail',
      })
      return
    }
        ...
        <BgCover color={props.color} coverRef={coverRef}>
          ...
          <div className='xl:min-w-[20rem]'>
            <div className='flex xl:flex-col items-end justify-center w-fit xl:px-10 space-x-8 xl:space-y-4 right-2'>
              ...
              <button
                className={
                  book.format?.ebook?.link ? 'btn-active' : 'btn-inactive'
                }
              ...
              <button
                className={
                  book.format?.audiobook?.link ? 'btn-active' : 'btn-inactive'
                }
        ...
        {!addReview && !editReview ? (
          <>
            ...
            <div className='w-full md:w-2/3 xl:w-1/2 p-4 md:p-8'>
              <h4 className='text-xl md:text-2xl font-semibold my-4'>
                Reviews
              </h4>
              <div className='flex flex-col gap-4 xl:gap-6'>
                {book.reviews?.length ? (
                  ...
                  <div className='rounded-md bg-[#152338] p-4'>
                    No reviews yet
                  </div>
                )}
...
export async function getStaticPaths() {
  ...
  return {
    paths: [], // Empty paths if no authors or data
    fallback: true,
  }
}
