  ...
  const followAuthorHandler = async () => {
    if (!user?.data) {
      snackbarCtx.addMessage({
        title: "Please login to save followed authors",
        status: "invalid",
      });
      return;
    }
    ...
    if (library.author === "followed")
      snackbarCtx.addMessage({
        title: "Author saved in your library",
        status: "success",
      });
    else
      snackbarCtx.addMessage({
        title: "Author removed from your library",
        status: "success",
      });
    setLoadingFollow(false);
  };
      ...
      <div className="cover-page-bg pb-16 xl:pb-8">
        ...
        {author.biography ? (
          <div className="p-4 md:p-6">
            <h4 className="text-xl md:text-2xl py-2 font-semibold">
              About the author
            </h4>
            <p
              ref={bioRef}
...
export async function getStaticPaths() {
  ...
  const authorParams = authors.data?.map((author) => ({
    params: { authorId: author.slug.toString() }, // Ensure authorId is a string
  }));

  return {
    paths: authorParams || [], // Ensure there's a default empty array if authorParams is null or undefined
    fallback: true,
  };
}
